...
class FNOPlus2d(nn.Module):
    def __init__(self, modes1, modes2,
        ...
        self.sp_convs = nn.ModuleList([SpectralConv2d(
            in_size+out_dim, out_size, mode1_num, mode2_num)
            for in_size, out_size, mode1_num, mode2_num
            in zip(self.layers, self.layers[1:], self.modes1, self.modes2)])

        self.ws = nn.ModuleList([nn.Conv1d(in_size*2, out_size, 1)
                                 for in_size, out_size in zip(self.layers, self.layers[1:])])
    ...
    def forward(self, x, y):
        ...
        y = y.permute(0,3,1,2)
        y = add_padding2(y, num_pad1, num_pad2)
        ...
        for i, (speconv, w) in enumerate(zip(self.sp_convs, self.ws)):
            ...
            x1 = speconv(x)
            x2 = w(x.view(batchsize, self.layers[i]*2, -1)).view(batchsize, self.layers[i+1], size_x, size_y)
            x = x1 + x2
            if i != length - 1:
                x = self.act(x)
        x = remove_padding2(x, num_pad1, num_pad2)
